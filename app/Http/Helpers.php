...
//////Filter Product by Distance
if (!function_exists('filter_products_lat_lng')) {
    function filter_products_lat_lng($products,$lat,$lng,$distance)
    {
        ...
        $verified_sellers = verified_sellers_id_by_lat_lng($lat,$lng,$distance);
        // dd($verified_sellers);
...
if (!function_exists('verified_sellers_id_by_lat_lng')) {
    function  verified_sellers_id_by_lat_lng($lat,$lng,$distance)
    {
            return Cache::rememberForever('verified_sellers_id_by_lat_lng', function () use ($lat,$lng,$distance) {
                $user_array_id=[]; 
                $shop_data = Shop::where('verification_status', 1)->pluck('user_id')->toArray();
                foreach($shop_data as $user_id)
                {
                    $user_data = User::find($user_id);
                    $lat_user = $user_data->lat;
                    $lng_user = $user_data->lng;
                    if($lat_user != null && $lng_user != null)
                    {
                        $distance_response = haversineGreatCircleDistance($lat_user, $lng_user, $lat, $lng);
                        if($distance_response <= $distance)
                        {
                            array_push($user_array_id,$user_id);
                        }
                    }
                }   
                return $user_array_id;
            });
    }
}
...
if (!function_exists('cart_product_price')) {
    function cart_product_price($cart_product, $product, $formatted = true, $tax = true)
    {
        if ($product->auction_product == 0) {
            ...
            $price = 0;
            if($cart_product['is_rfq'] == 1)
            {
                $price = $cart_product['price'];
            }
            else if($cart_product['is_order_sample'] == 1)
            {
                $price = $cart_product['price'];
            }
            else
            ...
            // additional cost
            if (empty($cart_product['is_rfq']) && $product->is_use_additional_cost) {
                $price += $product->additional_cost;
            }
        } else {
